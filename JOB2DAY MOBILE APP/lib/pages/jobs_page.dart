...
class _JobsPageState extends State<JobsPage> {
  ...
  List<Job> _filterLast24Hours(List<Job> jobs) {
    ...
    Logger.info('Filtering jobs from last 24 hours...');
    Logger.info('Current time: $now');
    Logger.info('24 hours ago: $twentyFourHoursAgo');
    
    final filtered = jobs.where((job) {
      final isToday = job.createdAt.isAfter(twentyFourHoursAgo);
      Logger.info('Job: \'${job.title}\' createdAt: \'${job.createdAt}\' - Today: $isToday');
      return isToday;
    }).toList();
    ...
    Logger.info('Found ${filtered.length} jobs from today out of ${jobs.length} total jobs');
    return filtered;
  }
                            ...
                            children: [
                              ...
                              Container(
                                decoration: BoxDecoration(
                                  ...
                                  boxShadow: [
                                    BoxShadow(
                                      color: const Color(0xFFFCEEEE).withValues(alpha: 0.4),
                                      spreadRadius: 1,
                                      blurRadius: 8,
                                      offset: const Offset(0, 2),
                                    ),
  ...
  Widget _buildJobCard(Job job) {
    return JobCard(
        ...
        onTap: () async {
          ...
          // Ads removed - no longer tracking visits for ads
