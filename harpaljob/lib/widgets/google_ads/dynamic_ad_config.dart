...
class DynamicAdConfig {
  ...
  static bool isEnabled(String adType) {
    ...
    if (!_apiConnectionSuccessful || _config == null) {
      Logger.info('DynamicAdConfig: isEnabled($adType) = true (fallback)');
      return true; // Fallback: enable banner when dynamic config not available
    }
    final enabled = _config?[adType]?['enabled'] ?? true; // default true for banner
  ...
  static String getAdUnitId(String adType) {
    ...
    if (!_apiConnectionSuccessful || _config == null) {
      // Fallback to Google test banner unit
      const testBanner = 'ca-app-pub-3940256099942544/6300978111';
      Logger.info('DynamicAdConfig: getAdUnitId($adType) = TEST($testBanner) (fallback)');
      return testBanner;
    }
  ...
  static T? getSetting<T>(String adType, String setting, [T? defaultValue]) {
    if (!isAvailable || adType != 'banner') return defaultValue;
  ...
  static bool shouldShowInterstitialOn(String pageType) { return false; }

  /// Get interstitial minimum interval
  static int getInterstitialMinInterval() { return 0; }
  ...
  static int getAppOpenMaxShowsPerDay() { return 0; }

  /// Get max ads per session
  static int getMaxAdsPerSession() { return 0; }

  /// Get cooldown period
  static int getCooldownPeriod() { return 0; }

  /// Get rewarded interstitial minimum interval
  static int getRewardedInterstitialMinInterval() { return 0; }

  /// Get rewarded interstitial max shows per session
  static int getRewardedInterstitialMaxShowsPerSession() { return 0; }
