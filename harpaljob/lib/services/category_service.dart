    ...
    try {
      ...
      if (response.data is List) {
        ...
        if (data.containsKey('value')) {
          // Laravel API returns {value: [...], Count: n}
          categoriesData = data['value'] as List<dynamic>;
        } else if (data.containsKey('categories')) {
          // Standard format with categories key
          categoriesData = data['categories'] as List<dynamic>;
        } else {
          // Try to parse the entire response as a list of categories
          categoriesData = data.entries.map((entry) {
        ...
        return {
          ...
          'total': data['Count'] ?? data['total'] ?? categoriesData.length,
          'page': data['page'] ?? page,
